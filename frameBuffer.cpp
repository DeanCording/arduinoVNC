...
void FrameBuffer::freeBuffer(void) {
    if(buffer) {
        //DEBUG_VNC("[FrameBuffer::fillRect] free: 0x%08X\n", buffer);
        free(buffer);
        buffer = 0;
        size = 0;
        //DEBUG_VNC("[FrameBuffer::fillRect] free: 0x%08X Done.\n", buffer);
    }
}

void FrameBuffer::fillRect(uint32_t x, uint32_t y, uint32_t rw, uint32_t rh, uint16_t color) {
    if(!buffer) {
        //DEBUG_VNC("[FrameBuffer::fillRect] buffer == null! <--------------------------------------\n");
        return;
    }

    if((((y + rh) * (x + rw)) * sizeof(color)) > size) {
        DEBUG_VNC("[FrameBuffer::fillRect] out of index!  <--------------------------------------\n");
        DEBUG_VNC("[FrameBuffer::fillRect] w: %d h: %d - x: %d y: %d rw: %d rh: %d color: 0x%04X\n", w, h, x, y, rw, rh, color);
        delay(50);
        return;
    }
    ...
    //DEBUG_VNC("[FrameBuffer::fillRect] offset: %d buffer: 0x%08X ptr: 0x%08X color: 0x%02X 0x%02X\n", offset, buffer, ptr, colorLow, colorHigh);
    //delay(10);
