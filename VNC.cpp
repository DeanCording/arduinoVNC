...
arduinoVNC::arduinoVNC(TFT_eSPI * _display) {
  host = "";
  port = 5900;
  display = _display;
  opt = {0};
...
void arduinoVNC::begin(char *_host, uint16_t _port, bool _onlyFullUpdate) {
  ...
  opt.client.width = display->width();
  opt.client.height = display->height();
  ...
  if (!connected()) {
    ...
    if ((millis() - lastUpdate) > updateDelay) {
      if (rfb_send_update_request(onlyFullUpdate ? 0 : 1)) {
        ...
        if (fails > 20) {
          disconnect();
          DEBUG_VNC("update_request excessive fails.\n");
...
bool arduinoVNC::read_from_rfb_server(int sock, uint8_t *out, size_t n) {
  ...
  /*
    ...
    if(n > 2 * 1024 * 1042) {
    DEBUG_VNC("read_from_rfb_server N: %d 0x%08X make no sense!\n", n, n);
    return 0;
    }
...
bool arduinoVNC::_rfb_negotiate_protocol() {
  ...
  if (!read_from_rfb_server(sock, (uint8_t *) &msg, sz_rfbProtocolVersionMsg))
    return false;
...
bool arduinoVNC::_read_conn_failed_reason(void) {
  ...
  if (!read_from_rfb_server(sock, (uint8_t *) &reason_length, sizeof(CARD32))) {
    return false;
  }
  ...
  if (!read_from_rfb_server(sock, (uint8_t *) reason_string, reason_length)) {
...
bool arduinoVNC::_read_authentication_result(void) {
  CARD32 auth_result;
  if (!read_from_rfb_server(sock, (uint8_t*) &auth_result, 4)) {
    return false;
  }
...
bool arduinoVNC::_rfb_authenticate() {
  ...
  if (protocolMinorVersion >= 7) {
    ...
    if (!read_from_rfb_server(sock, (uint8_t *) &nSecTypes, sizeof(nSecTypes))) {
      return false;
    }
    ...
    if (!read_from_rfb_server(sock, (uint8_t *) secTypes, nSecTypes)) {
      freeSec(secTypes);
      return false;
    }
    ...
    // protocol Minor < 7
    if (!read_from_rfb_server(sock, (uint8_t *) &authscheme, 4)) {
      return false;
    }
  ...
  switch (authscheme) {
    ...
    case rfbSecTypeVncAuth:
      ...
      if (!read_from_rfb_server(sock, (uint8_t *) challenge_and_response, CHALLENGESIZE)) {
        return false;
      }
...
bool arduinoVNC::_rfb_initialise_server() {
  ...
  if (!read_from_rfb_server(sock, (uint8_t *) &si, sz_rfbServerInitMsg)) {
    return false;
  }
  ...
  if (!read_from_rfb_server(sock, (uint8_t *)opt.server.name, len)) {
    return false;
  }
...
bool arduinoVNC::rfb_handle_server_message() {
  ...
  if (TCPclient.available()) {
    if (!read_from_rfb_server(sock, (uint8_t *) &msg, 1)) {
      return false;
    }
    digitalWrite(LED_BUILTIN, LOW);
    switch (msg.type) {
      case rfbFramebufferUpdate:
        read_from_rfb_server(sock, ((uint8_t *) &msg.fu) + 1, sz_rfbFramebufferUpdateMsg - 1);
        msg.fu.nRects = Swap16IfLE(msg.fu.nRects);
        for (uint16_t i = 0; i < msg.fu.nRects; i++) {
          read_from_rfb_server(sock, (uint8_t *) &rectheader,
...
bool arduinoVNC::_handle_server_cut_text_message(rfbServerToClientMsg * msg) {
  ...
  if (!read_from_rfb_server(sock, ((uint8_t *) &msg->sct) + 1, sz_rfbServerCutTextMsg - 1)) {
    return false;
  }
  ...
  buf = (uint8_t *) malloc((sizeof(uint8_t) * size) + 1);
  if (!buf) {
...
bool arduinoVNC::_handle_raw_encoded_message(rfbFramebufferUpdateRectHeader rectheader) {
  ...
  display->setWindow(rectheader.r.x, rectheader.r.y, rectheader.r.x + rectheader.r.w - 1, rectheader.r.y + rectheader.r.h - 1);
  ...
  freeSec(buf);
#endif
#ifdef DEBUG_VNC_UPDATE
  display->drawRect(rectheader.r.x, rectheader.r.y, rectheader.r.x + rectheader.r.w - 1, rectheader.r.y + rectheader.r.h - 1, TFT_BLUE);
#endif
...
bool arduinoVNC::_handle_copyrect_encoded_message(rfbFramebufferUpdateRectHeader rectheader) {
  ...
  if (!read_from_rfb_server(sock, (uint8_t *) &src_x, 2)) {
    return false;
  }

  src_x = (uint32_t)(Swap16IfLE(src_x));

  if (!read_from_rfb_server(sock, (uint8_t *) &src_y, 2)) {
    return false;
  }

  src_y = (uint32_t)(Swap16IfLE(src_y));
...
#ifdef VNC_SAVE_MEMORY
    int max_buffer_pixels = (ESP.getFreeHeap() / sizeof(uint16_t)) / 4;  
#else
    int max_buffer_pixels = (ESP.getFreeHeap() / sizeof(uint16_t)) / 2;
#endif

  int diff_x = rectheader.r.x - src_x;
  int diff_y = rectheader.r.y - src_y;

  uint16_t * colorBuffer = NULL;

  if (abs(diff_x) > abs(diff_y)) {
    // Copy in X direction

    int cols = min((max_buffer_pixels / rectheader.r.h), rectheader.r.w);

    if (cols < 1) return false;
    
    colorBuffer = (uint16_t *)malloc(cols * rectheader.r.h * sizeof(uint16_t));

    if (diff_x < 0) {
      // Copy R->L
      int segment;
      for (segment = 0; segment + cols < rectheader.r.w; segment += cols) {
      ...
      if (last_segment > 0) {
        display->readRect(src_x + segment, src_y, (uint32_t)last_segment, (uint32_t)rectheader.r.h, colorBuffer);
      ...
      // Copy L->R
      int segment;
      for (segment = rectheader.r.w - cols; segment >= 0; segment -= cols) {
        display->readRect(src_x + segment, src_y, (uint32_t)cols, (uint32_t)rectheader.r.h, colorBuffer);
      ...
      if (last_segment > 0) {
        display->readRect(src_x, src_y, (uint32_t)last_segment, (uint32_t)rectheader.r.h, colorBuffer);
    ...
    int rows = min((max_buffer_pixels / rectheader.r.w), rectheader.r.h);

    if (rows < 1) return false;

    colorBuffer = (uint16_t *)malloc(rows * rectheader.r.w * sizeof(uint16_t));

    if (diff_y < 0) {
      // Copy up
      int segment;
      for (segment = 0; segment + rows <= rectheader.r.h; segment += rows) {
        display->readRect(src_x, src_y + segment, (uint32_t)rectheader.r.w, (uint32_t)rows, colorBuffer);

        display->pushImage((uint32_t)rectheader.r.x, (uint32_t)rectheader.r.y + segment,
                           (uint32_t)rectheader.r.w, (uint32_t)rows, colorBuffer);
      }

      int last_segment = rectheader.r.h  - segment;

      if (last_segment > 0) {
        display->readRect(src_x, src_y+segment, (uint32_t)rectheader.r.w, (uint32_t)last_segment, colorBuffer);
      ...
      // Copy down
      int segment;
      for (segment = rectheader.r.h - rows; segment >= 0; segment -= rows) {
        display->readRect(src_x, src_y + segment, (uint32_t)rectheader.r.w, (uint32_t)rows, colorBuffer);
      ...
      int last_segment = segment + rows;

      if (last_segment > 0) {
        display->readRect(src_x, src_y, (uint32_t)rectheader.r.w, (uint32_t)last_segment, colorBuffer);
  ...
  free(colorBuffer);
#ifdef DEBUG_VNC_UPDATE
  display->drawRect(src_x, src_y, (uint32_t)rectheader.r.w, (uint32_t)rectheader.r.h, TFT_DARKGREEN);
  display->drawRect((uint32_t)rectheader.r.x, (uint32_t)rectheader.r.y,
         (uint32_t)rectheader.r.w, (uint32_t)rectheader.r.h, TFT_GREEN);
#endif
...
bool arduinoVNC::_handle_rre_encoded_message(rfbFramebufferUpdateRectHeader rectheader) {
  ...
  display->fillRect(rectheader.r.x, rectheader.r.y, rectheader.r.w, rectheader.r.h, color);
  ...
  for (uint32_t i = 0; i < header.nSubrects; i++) {
    ...
    if (!read_from_rfb_server(sock, (uint8_t *) &rect, sizeof(rect)))
      return false;
    display->fillRect(
      Swap16IfLE(rect[0]) + rectheader.r.x,
      Swap16IfLE(rect[1]) + rectheader.r.y, Swap16IfLE(rect[2]), Swap16IfLE(rect[3]), color);
  }

#ifdef DEBUG_VNC_UPDATE
  display->drawRect(rectheader.r.x, rectheader.r.y, rectheader.r.w, rectheader.r.h, TFT_YELLOW);
#endif
  return true;
}
#endif
...
bool arduinoVNC::_handle_corre_encoded_message(rfbFramebufferUpdateRectHeader rectheader) {
  ...
  for (uint32_t i = 0; i < header.nSubrects; i++) {
    ...
    display->fillRect(rect[0] + rectheader.r.x, rect[1] + rectheader.r.y, rect[2], rect[3], color);
  }
  return true;
}
#endif
...
bool arduinoVNC::_handle_hextile_encoded_message(rfbFramebufferUpdateRectHeader rectheader) {
...
#ifdef VNC_SAVE_MEMORY
  uint8_t * buf = (uint8_t *) malloc(255 * sizeof(HextileSubrectsColoured_t));
#else
  ...
  while ((i * 16) < rect_h) {
    ...
    while ((j * 16) < rect_w) {
      ...
      if (!read_from_rfb_server(sock, (uint8_t *) &subrect_encoding, 1)) {
#ifdef VNC_SAVE_MEMORY
        freeSec(buf);
#endif
        return false;
      ...
      if (subrect_encoding & rfbHextileRaw) {
...
#ifdef DEBUG_VNC_UPDATE
        display->drawRect(rect_xW, rect_yW, tile_w, tile_h, TFT_PINK);
#endif
        ...
        if (subrect_encoding & rfbHextileBackgroundSpecified) {
          if (!read_from_rfb_server(sock, (uint8_t *) &bgColor, sizeof(bgColor))) {
#ifdef VNC_SAVE_MEMORY
            freeSec(buf);
#endif
            return false;
        ...
        if (subrect_encoding & rfbHextileForegroundSpecified) {
          if (!read_from_rfb_server(sock, (uint8_t *) &fgColor, sizeof(fgColor))) {
#ifdef VNC_SAVE_MEMORY
            freeSec(buf);
#endif
            return false;
        ...
        if (subrect_encoding & rfbHextileAnySubrects) {
          uint8_t nr_subr = 0;
          if (!read_from_rfb_server(sock, &nr_subr, 1)) {
#ifdef VNC_SAVE_MEMORY
            freeSec(buf);
#endif
            return false;
...
#endif
#ifdef DEBUG_VNC_UPDATE
        display->drawRect(rect_xW, rect_yW, tile_w, tile_h, TFT_RED);
#endif
      }
      j++;
      delay(0);
